...
texte = creer_figure (texte)
	argument: un texte au format str contenant des figures
...
txt_file_to_html_file (fichier_txt, fichier_css=None):
	arguments:
...
def txt_file_to_html_file (fichier_txt, fichier_css=None):
	"""
	arguments:
		un fichier .txt contenant un texte avec ma mise en forme
		(facultatif) un fichier .css contenant les styles a appliquer
...
#	transformer le texte en texte html
	texte = txt.lire (fichier_txt)
	texte = modif_texte (texte)
#	titre du fichier
	f= fichier_txt.rfind (ossep) +1	# reperer le debut du nom du fichier
...
#	creer le fichier html
	...
	txt.ecrire (fichier_html, contenu_html, 'w')
